 ...
 *
 * $Source: /cvs_backup/e107_0.8/e107_plugins/dealership/e_sitelink.php,v $
 * $Revision$
 * $Date$
 * $Author$
 *
...
class dealership_sitelink // include plugin-folder in the name.
{
	function config()
	{
		global $pref;
	...
	function myCategories()
	{
		...
		$sql->select("dealership","*","dealership_id != '' ");
		
		while($row = $sql->fetch())
		{
			$sublinks[] = array(
				'link_name'			=> $tp->toHTML($row['dealership_name'],'','TITLE'),
				'link_url'			=> e107::url('dealership', 'other', $row),
				'link_description'	=> '',
				'link_button'		=> $row['dealership_icon'],
				'link_category'		=> '',
				'link_order'		=> '',
				'link_parent'		=> '',
				'link_open'			=> '',
