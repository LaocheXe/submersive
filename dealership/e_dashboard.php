...
class dealership_dashboard // include plugin-folder in the name.
{
	...
	function activity()
	{
		...
		if(!$sql->gen("SELECT dealership_id,dealership_date,dealership_folder FROM `#dealership` WHERE dealership_date BETWEEN ".$month_start." AND ".$month_end))
		{
			return false;
		}
		...
		while($row = $sql->fetch())
		{
			...
			$key = date('Y-n-j', $row['dealership_date']);

			switch($row['dealership_folder'])
			{
				case "Facebook":
					$amt[$key]['facebook'] += 1;
					break;
			...
			$dateName[$key] = date('jS', $row['dealership_date']);
		}
	...
	{
		$var[0]['icon'] 	= "<img src='".e_PLUGIN."dealership/images/dealership_16.png' alt='' />";
		$var[0]['title'] 	= "My Title";
		$var[0]['url']		= e_PLUGIN_ABS."dealership/dealership.php";
		$var[0]['total'] 	= 10;
